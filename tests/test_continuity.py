...
def test_continuity_uses_rowid_watermark_and_filters_person_refs(tmp_path: Path):
    ...
    store.add_manual_memory(
        "space", ManualMemoryRequest(
            content="Alice is a colleague.", people=["Alice"])
    )
    ids = store.record_messages(
        "space", [MessageInput(
            author="user", content=f"message {i}") for i in range(3)]
    )
    ...
    alice_id = store.read("space", QueryRequest(
        question="dossier", people=["Alice"])).people[0].id
    assert store.apply_continuity_reasoning(
        "space", ids[1],
        ContinuityReasoningResult(
...
def test_continuity_context_can_delegate_all_batching_to_reasoner(tmp_path: Path):
    ...
    ids = store.record_messages(
        "space", [MessageInput(
            author="user", content=f"m{i}") for i in range(40)]
    )
...
def test_continuity_truncates_oversized_messages_without_stalling(tmp_path: Path):
    ...
    while True:
        ...
        expected_through = result.through_message_id
    assert calls_data
    assert all(max(len(message.content)
               for message in batch) <= 8000 for batch in calls_data)
    assert all(sum(len(message.content)
               for message in batch) <= 48000 for batch in calls_data)
        ...
        async with app.router.lifespan_context(app):
            ...
            async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
                ...
                assert set(response.json()) == {
                    "version", "user_model", "continuity", "people", "guidance"}
